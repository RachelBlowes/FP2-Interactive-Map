  ...
  function createChart(data, width, height) {
    ...
    .style("fill", "none");
        ...
        var y = d3.scaleLinear()
            .domain([0, d3.max(data, function(d) { return +d['total buyers']; })])
            .range([height, 0]);
        svg.append("g")
            .call(d3.axisLeft(y));
            svg.selectAll("mybar")
            ...
            .duration(800)
            .attr("y", function(d) { return y(+d['total buyers']); })
            .attr("height", function(d) { return height - y(+d['total buyers']); })
        ...
        svg.selectAll("mybar")
            ...
            .append("rect")
                ...
                .on('mouseenter', function(event, d) {
                    tooltip
                        .html(`<div>Tract: ${d.tract}</div><div>Number of permits: ${d['total buyers']}</div><div>MHI: ${d['median household income']+"$"}</div>`)
                        .style('visibility', 'visible')
                        .style('top', (event.pageY - y(+d['total buyers']) - margin.top) + 'px') // Adjust top position relative to the bar
